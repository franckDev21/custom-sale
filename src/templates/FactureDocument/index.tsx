...
import { formatCurrency, pttc, user } from "../../utils/function";
      ...
      <Page wrap size="A4" style={styles.page}>
            ...
            <View style={styles.content}>
              <View style={styles.customer}>
                ...
                <Text style={{ marginBottom: 4 }}>
                  N° : {order.invoice?.reference}
                </Text>
                <Text style={{ marginBottom: 4 }}>
                  Date : {dayjs().format("DD/MM/YYYY")}
                </Text>
                <Text style={{ marginBottom: 4 }}>
                  Tél du vendeur : {order.customer?.tel}
                </Text>
                <Text style={{ marginBottom: 4 }}>
                  Nom du vendeur : {order.user?.firstname}{" "}
                  {order.user?.lastname}
                </Text>
              </View>
            </View>
          ...
          <View style={styles.head}>
            <View style={styles.content}>
              <Text style={styles.date}>
                Douala {dayjs().format("DD MMMM YYYY")}
              </Text>
              <View style={styles.customer}>
                ...
                <Text style={{ marginBottom: 3 }}>
                  Tél : {order.customer?.tel}
                </Text>
                <Text style={{ marginBottom: 3 }}>
                  Localisation : {order.customer?.address}
                </Text>
              </View>
            </View>
          </View>
        ...
        <View style={styles.table}>
          ...
          <View style={styles.tableTotal}>
            <Text>
              {formatCurrency(parseInt(order.total_ht || "0", 10) || 0, "XAF")}
            </Text>
          </View>
          ...
          {order.invoice?.as_tva && (
            <>
              <View style={styles.tableTotalLabel}>
                <Text style={{ textAlign: "right" }}>Total TVA (19.5%)</Text>
              </View>
              <View style={styles.tableTotal}>
                <Text>
                  {formatCurrency(
                    pttc(parseInt(order.total_ht || "0", 10)).totalTVA,
                    "XAF"
                  )}
                </Text>
              </View>
          ...
          {order.invoice?.as_ir && (
            <>
              <View style={styles.tableTotalLabel}>
                <Text style={{ textAlign: "right" }}>Total IR (5.5%)</Text>
              </View>
              <View style={styles.tableTotal}>
                <Text>
                  {formatCurrency(
                    pttc(parseInt(order.total_ht || "0", 10)).totalIR,
                    "XAF"
                  )}
                </Text>
              </View>
