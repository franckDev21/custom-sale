              ...
              <>
                ...
                <div className="flex space-x-4 font-bold items-center ml-4">
                  <button
                    ...
                    className="text-sm text-white px-4 rounded-md bg-red-400 py-2"
                  >
                    {" "}
                    <FaMinus size={16} className="inline-block  mr-1" />
                    Ajouter une sortie
                  </button>
                  <button
                    onClick={(_) => {
      ...
      <div className="mx-auto max-w-7xl py-6 sm:px-6 lg:px-8">
              ...
              <form onSubmit={confirmAddOutput} className="text-left">
                ...
                <div className="flex flex-col space-y-4 mt-4 mb-6">
                  <div>
                    <label
                      htmlFor="quantite"
                      className="inline-block mb-2 font-semibold text-gray-700"
                    >
                      Quantité
                    </label>
                    <input
                      ...
                      min={0}
                      className="w-full px-3 placeholder:italic py-2 bg-slate-100 rounded-md outline-none border-none ring-0 focus:ring-2 focus:ring-gray-600"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      name="motif"
                      onChange={handleOnchange}
                      value={dataOutputForm.motif || ""}
                      required
                      id="quantite"
                      placeholder="Entrer le motif du retrait"
                      className="w-full px-3 placeholder:italic py-2 bg-slate-100 rounded-md outline-none border-none ring-0 focus:ring-2 focus:ring-gray-600"
                      rows={6}
                    ></textarea>
                  </div>
                ...
                <div className="flex justify-center space-x-3">
                  <button
                    ...
                    className={`bg-green-500 ${
                      sending2 && "disabled"
                    } hover:bg-green-700 transition text-white rounded-md px-3 font-semibold uppercase py-2 w-1/2 inline-block`}
                  >
                    {sending2 ? (
                ...
                <div className="flex flex-col space-y-4 mt-4 mb-6">
                  <div>
                    ...
                    <input
                      name="quantite"
                      onChange={handleOnchange}
                      value={dataInputForm.quantite || ""}
                      required
                      autoFocus
                      type="number"
                      id="quantite"
                      placeholder="Entrer la quantité d’approvisionnement"
                      min={0}
                      className="w-full px-3 placeholder:italic py-2 bg-slate-100 rounded-md outline-none border-none ring-0 focus:ring-2 focus:ring-gray-600"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      name="prix_achat"
                      onChange={handleOnchange}
                      value={dataInputForm.prix_achat || ""}
                      required
                      type="number"
                      id="price"
        ...
        {loading ? (
          ...
          <div className="max-w-5xl bg-white p-5 rounded-md mx-auto flex space-x-5 relative overflow-hidden">
            <Link
              to={`/products/edit/${product.id}/${product.name
                ?.split(" ")
                .join("-")
                .toLowerCase()}`}
            ...
            <div className=" min-h-[400px]  w-[300px]">
              ...
              <div className="mt-4 space-y-6 border-t pt-4">
                <h2>
                  ...
                  <span
                    className={`px-3 py-2 text-white ${
                      product.is_stock ? "bg-green-500" : "bg-red-500"
                ...
                <h2>
                  <span className="font-semibold">Stock d'alerte </span>:{" "}
                  <span
                    className={`px-3 py-2 text-red-500 rounded-full opacity-70`}
                  >
                    {product.qte_stock_alert} {product.type_approvisionnement}
                    {(product.qte_stock_alert || 0) > 1 && "s"}
                  </span>
                </h2>
              </div>
            </div>
            <div className="w-[calc(100%-300px)] flex flex-col">
              ...
              <div className="flex items-center space-x-3 mt-3 text-xl">
                ...
                <div className="p-2 rounded-md bg-slate-100">
                  {product.qte_en_stock} {product.type_approvisionnement}
                  {(product.qte_en_stock || 0) > 1 && "s"}
                  {product.product_type?.name ===
                    "VENDU_PAR_NOMBRE_PAR_CONTENEUR" &&
                    ` de ${product.nbre_par_carton} et ${
                      product.unite_restante || 0
                    } Unité${
                      (product.unite_restante || 0) > 1 ? "s" : ""
                    } restante`}
                  {product.product_type?.name === "VENDU_PAR_LITRE" &&
                    ` de ${product.qte_en_litre} ${
                      product.product_type.unite_de_mesure
                    } et ${product.unite_restante || 0} Unité${
                      (product.unite_restante || 0) > 1 ? "s" : ""
                    } restante`}
                  {product.product_type?.name === "VENDU_PAR_KG" &&
                    ` de ${product.poids} ${
                      product.product_type.unite_de_mesure
                    } et ${product.unite_restante || 0} Unité${
                      (product.unite_restante || 0) > 1 ? "s" : ""
              ...
              <div className="flex items-center space-x-3 mt-3 text-xl">
                ...
                <div className="p-2 rounded-md bg-slate-100">
                  {formatCurrency(
                    parseInt(product.prix_unitaire?.toString() || "0", 10) || 0,
                    "XAF"
                  )}
                </div>
              </div>
              ...
              <div className="flex space-x-4 mt-3 items-center">
                <div className="flex items-center space-x-3 text-xl">
                  ...
                  <div className="p-2 rounded-md text-[#ac3265]">
              ...
              <div className="flex">
                <button
                  ...
                  className="text-sm text-white px-4 rounded-md bg-primary py-2"
                >
                  Ajouter un nouvelle approvisionnement
                </button>
              </div>
