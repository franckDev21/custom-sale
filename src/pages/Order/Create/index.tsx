...
const OrderCreate = () => {
    ...
    http_client(Storage.getStorage("auth").token)
      .post(
        companiesStore.currentCompany
          ? `${CREATE_ORDER_URL}?id=${companiesStore?.currentCompany?.id}`
          : CREATE_ORDER_URL,
        data
      )
      .then((res) => {
  ...
  const addToCart = (product: ProductCart) => {
    ...
    if (!exist) {
      ...
      if (product.product_type?.name === "VENDU_PAR_KG") {
        nbreUnites =
          (parseInt(product.qte_en_stock?.toString() || "0", 10) || 0) *
            (parseInt(product.poids?.toString() || "0", 10) || 0) +
          (parseInt(product.unite_restante?.toString() || "0", 10) || 0);
        ...
        nbreUnites =
          (parseInt(product.qte_en_stock?.toString() || "0", 10) || 0) *
            parseInt(product.qte_en_litre?.toString() || "0", 10) +
          (parseInt(product.unite_restante?.toString() || "0", 10) || 0);
      } else if (
        product.product_type?.name === "VENDU_PAR_NOMBRE_PAR_CONTENEUR"
      ) {
        nbreUnites =
          (parseInt(product.qte_en_stock?.toString() || "0", 10) || 0) *
            parseInt(product.nbre_par_carton?.toString() || "0", 10) +
          (parseInt(product.unite_restante?.toString() || "0", 10) || 0);
      }
      ...
      newProduct = {
        ...
        prix_de_vente:
          product.product_type?.name === "VENDU_PAR_PIECE"
            ...
            : null,
        type_de_vente:
          product.product_type?.name !== "VENDU_PAR_PIECE" ? "DETAIL" : "PIECE",
      };
      ...
      if (produitFind) {
        produitFind.qte =
          (parseInt(produitFind.qte?.toString() || "0", 10) || 0) + 1;
    ...
    if (produitFind) {
      if (value === "DETAIL") {
        ...
        if (produitFind.product_type?.name === "VENDU_PAR_KG") {
          nbreUnites =
            (parseInt(produitFind.qte_en_stock?.toString() || "0", 10) || 0) *
              (parseInt(produitFind.poids?.toString() || "0", 10) || 0) +
            (parseInt(produitFind.unite_restante?.toString() || "0", 10) || 0);
          ...
          nbreUnites =
            (parseInt(produitFind.qte_en_stock?.toString() || "0", 10) || 0) *
              parseInt(produitFind.qte_en_litre?.toString() || "0", 10) +
            (parseInt(produitFind.unite_restante?.toString() || "0", 10) || 0);
        } else if (
          produitFind.product_type?.name === "VENDU_PAR_NOMBRE_PAR_CONTENEUR"
        ) {
          nbreUnites =
            (parseInt(produitFind.qte_en_stock?.toString() || "0", 10) || 0) *
              parseInt(produitFind.nbre_par_carton?.toString() || "0", 10) +
            (parseInt(produitFind.unite_restante?.toString() || "0", 10) || 0);
        }
      } else {
  ...
  const isValid = () => {
    ...
    carts.forEach((product) => {
      ...
      prix +=
        (parseInt(product.qte?.toString() || "0", 10) || 0) *
        (parseInt(product.prix_de_vente?.toString() || "0", 10) || 0);
    });
    return {
      ok: som > 0 && client !== "",
  ...
  const search = () => {
    if (filter !== "") {
      let newTab = products.filter(
        (product) =>
          (product.name || "").toLowerCase().includes(filter.toLowerCase()) ||
          (product.type_approvisionnement || "")
            .toLowerCase()
    ...
    const fetUsers = async () => {
      const res = await Promise.all([
        http_client(Storage.getStorage("auth").token).get(
          companiesStore.currentCompany
            ? `${GET_MY_CUSTOMERS}?id=${companiesStore?.currentCompany?.id}`
            : GET_MY_CUSTOMERS
        ),
        http_client(Storage.getStorage("auth").token).get(
          companiesStore.currentCompany
            ? `${GET_PRODUCTS}?id=${companiesStore?.currentCompany?.id}`
            : GET_PRODUCTS
        ),
      ]);
            ...
            {success ? (
              <>
                <div className="flex flex-col py-16 justify-center w-full items-center space-y-5">
                  ...
                  <h3 className="text-3xl text-gray-600 w-[40%] text-center mx-auto font-bold">
                    La commande a été enregistrée avec succès !
                  </h3>
                  <p>
                    La commande de{" "}
                    <span className="text-primary">
                      {getCustomer(client || "1")?.firstname}{" "}
                      {getCustomer(client || "1")?.lastname}
                    </span>{" "}
                    a été enregistré{" "}
                  </p>
                  <div className="flex items-center justify-center space-x-4">
                    ...
                    <Link
                      to={`/orders/show/${orderId || 0}/1659495`}
              ...
              <>
                ...
                {showClientForm && (
                  <CustomerForm
                    onClickBack={(value) => setShowClientForm(value)}
                    addNewClient={(value) => {
                ...
                <form onSubmit={commander} className="w-1/2 ml-2">
                  <div className="relative z-50">
                    <input
                      ...
                      placeholder="Réchercher un produit ici"
                    ...
                    <div className="bg-gray-300 absolute w-full flex-col flex justify-start items-start top-full left-0 ring-0 rounded-b-md">
                      {productSearch.map((product: Product) => (
                        <div
                          ...
                          key={product.id}
                          className="w-full z-50 flex justify-start items-start mb-2 p-1 hover:bg-gray-200 transition cursor-pointer"
                        >
                          <div className="w-10 relative h-10 overflow-hidden">
                            <img
                              src={`${
                                product.image
                                  ? `${API_STORAGE_URL}/${product.image}`
                                  : "/static/img/product.png"
                              }`}
                              className=" absolute z-0 h-auto w-full object-cover"
                              alt="imageproduit"
                            />
                          </div>
                          <div className="ml-2 -translate-y-1">
                            ...
                            <h2 className="font-semibold text-sm text-primary">
                              ...
                              {formatCurrency(
                                parseInt(
                                  product.prix_unitaire?.toString() || "0",
                                  10
                                ) || 0
                              )}
                              {product.is_stock ? (
                                <>
                                  <span className="px-2 font-normal rounded-lg mx-2 py-1 text-xs bg-green-100 text-green-500">
                                    En stock
                                  </span>{" "}
                                  | {product.qte_en_stock}{" "}
                                  {product.type_approvisionnement}(s)
                                </>
                              ) : (
                                <span className="px-2 font-normal rounded-lg mx-2 py-1 text-xs bg-red-100 text-red-500">
                                  stock vide
                                </span>
                              )}
                            </h2>
                  ...
                  <div className="mt-3 z-0">
                    {carts.length > 0 ? (
                      <>
                        {carts.map((product, i) => (
                          <div
                            key={`${product.id}-${i}`}
                            className="w-full flex justify-start items-start mb-2 p-1 relative z-10 "
                          >
                            <span
                              onClick={() => deleteProduct(product.id || "0")}
                              className="fa-solid fa-times absolute right-2 cursor-pointer inline-block ml-2 bg-red-100 p-2 top-2 text-red-500 z-50"
                            >
                              <TiTimes />
                            </span>
                            ...
                            {product.product_type?.name !==
                              ...
                              <select
                                ...
                                className="absolute right-14 top-2 appearance-none px-8 text-sm text-center font-bold ml-2 py-0.5 bg-gray-100 rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-gray-600 outline-none border-none"
                              >
                            ...
                            <div className="ml-2 -translate-y-1">
                              ...
                              <h2 className="font-semibold text-sm text-primary">
                                Prix unitaire {product.type_approvisionnement} :{" "}
                                {formatCurrency(
                                  parseInt(
                                    product.prix_unitaire?.toString() || "0",
                                    10
                              ...
                              <div>
                                <input
                                  ...
                                  max={product.max}
                                  value={
                                    findProduct(product.id || "0")?.qte || 0
                                  }
                                  onChange={(e) =>
                                    setQte(product.id || "0", e.target.value)
                                  }
                                  type="number"
                                  className="mt-1 appearance-none px-1 text-center font-bold w-20 py-1 bg-gray-100 rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-gray-600 outline-none border-none"
                                  required
                                />
                                <input
                                  min={0}
                                  value={
                                    findProduct(product.id || "0")
                                      ?.prix_de_vente || ""
                                  }
                                  onChange={(e) =>
                                    setPriceShop(
                                  ...
                                  type="number"
                                  className="mt-1 appearance-none pl-2 pr-1 font-bold ml-2 py-1 bg-gray-100 rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-gray-600 outline-none border-none"
                                  required
                                />
                              </div>
                            </div>
                      ...
                      <div className="min-h-[200px] text-center text-gray-500 text-2xl bg-slate-100 rounded-md flex flex-col justify-center items-center">
                        ...
                        <span className="mb-2 mt-1">
                          Aucun produit séléctionner{" "}
                        </span>
                        <span className="text-base px-5">
                          veillez réchercher le(s) et cliquez dessus pour les
                          ajoutes dans la commande
                        </span>
                      </div>
                    )}
                    <div className="my-4 border-y flex justify-end items-end flex-col-reverse py-2">
                      ...
                      <select
                        ...
                        className="mt-1 appearance-none px-8 text-center font-bold py-1 bg-gray-100 rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-gray-600 outline-none border-none"
                      >
                    ...
                    <div className="py-3">
                      <button
                        className={`bg-primary  ${
                          (!isValid().ok || sending) && "disabled"
                        } border-2 border-primary transition text-white px-6 py-2 w-full rounded-md`}
                      >
                        {sending ? (
