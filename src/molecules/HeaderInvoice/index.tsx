...
const HeaderInvoice: React.FC<HeaderInvoiceProps> = ({ companyId = 1 }) => {
  const [company, setCompany] = useState<Company>({});

  useEffect(() => {
    http_client(Storage.getStorage("auth").token)
      .get(`${GET_COMPANY_URL}/${companyId}`)
      .then((res) => {
        setCompany(res.data);
      })
      .catch((err) => {
    ...
    <View style={styles.header}>
      ...
      <View style={styles.right}>
        <View style={styles.rightBottom}>
          <Text style={{ padding: 4, marginLeft: 5 }}>{company.name}</Text>
        </View>
        <View style={styles.rightBottom}>
          <Text style={{ padding: 4, marginLeft: 5 }}>
            Commerce Général - Prestations de Services
          </Text>
        </View>
